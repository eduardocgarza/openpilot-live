...
export default class TripFull implements ITripFull {
  ...
  constructor(jsonFile: TripJSON, tripFile: TripFile, location: TripLocation) {
    ...
    // Movement Data
    const { timeIdle, timeMoving, percentageIdle, percentageMoving } = this.calculateMovement(coordinates)
  ...
  private calculateAverages () {
    ...
    const accelerations = coords.reduce ((sum, coord) => sum + coord.acceleration, 0)
  ...
  private static async getLocation(coords: TripCoordinate[]) {
    ...
    let startLocation = "", endLocation = "", country = "", city = ""
  ...
  public static generateCoordinates (coords: ITripJSONFileCoordinate[]) {
    return coords.map ((coord, index) => {
      ...
      let acceleration = (index === 0) ? 0 : coord.speed - coords[index-1].speed
      const { index: idx, dist: accummulatedDistance, speed, lng, lat } = coord
      return new TripCoordinate(idx, dist, speed, lat, lng, accummulatedDistance, acceleration)
    })
  }
